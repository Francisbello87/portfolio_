...
const Tech = () => {
  return (
    <div name="tech" className="w-full md:h-screen bg-bgColor pt-10 ">
      <div className="max-w-[1000px] mx-auto px-4 flex flex-col justify-center  sm:text-left h-full">
        <div className="pb-4">
        ...
        <div className="w-full grid grid-cols-2 sm:grid-cols-3 gap-4 text-center py-8 text-gray-300">
          {TechStacks.map((techstack) => (
            <div
              key={techstack.id}
              className="shadow-md group pointer-events-none bg-slate-800 rounded-md shadow-bgColor py-4 "
            >
              <img
                ...
                alt="HTML Icon"
              />
              <p className="mt-4 group-hover:text-primaryColor group-hover:font-bold">
                {techstack.title}
              </p>
            </div>
          ))}
